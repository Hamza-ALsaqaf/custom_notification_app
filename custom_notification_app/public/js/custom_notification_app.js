...
frappe.ui.Notifications = class NotificationsOveride extends frappe.ui.Notifications {
	setup_headers() {
		// Add header actions
		...
		this.categories = [
			{
				...
				id: "notifications",
				view: NotificationsViewOveride,
				el: this.panel_notifications,
			},
			{
				label: __("Events"),
				id: "todays_events",
				view: EventsViewOveride,
				el: this.panel_events,
			},
			{
				label: __("What's New"),
				id: "changelog_feed",
				view: ChangelogFeedViewOveride,
...
class NotificationsViewOveride extends BaseNotificationsView {
	make() {
		...
		this.get_notifications_list(this.max_length).then((r) => {
			...
			if (this.settings.seen == 0 && this.dropdown_items.length > 0) {
                ...
                // console.log("this is custom_enable_sound:"+this.settings.custom_enable_sound)
    ...
    {
        if(this.settings.custom_enable_sound==1)
        {
            frappe.utils.play_sound("alert");   
        }
        else{
			
			if(this.settings.custom_enable_alert_message==1)
				{
				// frappe.show_alert({ message: __("{0} You have a new notification", [first_name]), indicator: "green" });
				frappe.show_alert({ message: __("You have a new notification"),seconds: 10, indicator: "green" });
...
class EventsViewOveride extends BaseNotificationsView {
	make() {
...
class ChangelogFeedViewOveride extends BaseNotificationsView {
	make() {
